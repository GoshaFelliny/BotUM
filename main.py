...
async def send_reminders(app):
    ...
    try:
        ...
        teachers = session.query(Teacher).filter(
            ...
            Teacher.registration_time <= current_time - timedelta(minutes=1)
        ).all()
...
async def main() -> None:
    ...
    # Передаем в job функцию с контекстом бота через lambda, чтобы получить context.bot
    scheduler.add_job(lambda: asyncio.create_task(send_reminders(app)), IntervalTrigger(minutes=1))
    scheduler.start()
...
if __name__ == '__main__':
    try:
        ...
        # Если event loop уже запущен (например, в Jupyter/IDE), используем его
